...
4. To use the data in ArcGIS Pro, `extract` or unzip the shapefile, and drag the file ending in `.shp` into the ArcGIS project document/map. Alternatively, you can drag the `.geoJSON` file into the ArcGIS Pro document. Another way to add vector data to an ArcGIS Pro project is use the menu on the top bar `Map` → `Add Data` → Navigate through your computer folder structure to see the `shapefile` you'd like to add.
...
6. Right-click `Low_Food_Access_Areas` in the Layers pane → `Symbology` → `Primary symbology` = `Graduated colors` → `Field` = `PRTUND185` → Under `Method` notice there are many classification options. Be aware of which you’re choosing by [reading about each method](https://pro.arcgis.com/en/pro-app/latest/help/mapping/layer-properties/data-classification-methods.htm).