...
> 6. Notice the `Download GeoTIFF` button which gives a file that will work in ArcGIS Pro.
...
3. For the class demo we will georeference this map using [AllMaps](https://editor.allmaps.org/), a tool for georeferencing maps from library collections in a web browser.
4. If the scanned image of the map is not streaming online via HOLLIS, you will need to obtain a hard copy of the image on your computer and use ArcGIS Pro to georeference it. The sample layer included with this activity, `G7625_1897_W2.tif` is an example of this process. <br>[Make an appointment](https://library.harvard.edu/libraries/harvard-map-collection) ; [ESRI documentation → Georeference a raster to a referenced layer](https://pro.arcgis.com/en/pro-app/latest/help/data/imagery/georeferencing-a-raster-to-a-referenced-layer.htm).
